...
## Calculating count for each category

cat_cnt  <- function(data, category) {
  ...
  small_n_cnt <- data  %>%
    group_by(TRT01AN, TRT01A, {{category}}) %>%
    count(name = "n")
...
small_n_cnt %>%
  ...
  ungroup() %>% 
  select(TRT01A, {{category}}, npct) %>% 
  pivot_wider(names_from = TRT01A, 
              values_from = npct)

}

adsl_mrg_cnt <- cat_cnt(adsl_saf, SEX)
adsl_mrg_cnt <- cat_cnt(adsl_saf, AGEGR1)
adsl_mrg_cnt <- cat_cnt(adsl_saf, RACE)
